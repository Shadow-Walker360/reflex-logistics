...
/**
 * Captures an OTP/confirmation code and submits it for backend
 * verification (Section 19 of the frontend spec: the frontend captures
 * input, it does not itself decide a delivery is complete). The exact
 * proof mechanism (OTP vs QR vs signature vs photo) is unconfirmed;
 * OTP-style text entry is used here as the simplest placeholder UI that's
 * easy to swap for a different capture method once decided — see
 * docs/api-contract.md for the proposed request shape.
 *
 * Goes through deliveryService.confirmDelivery (POST
 * /deliveries/:id/proof-of-delivery, per FULL_SCALE_DELIVERY_DIRECTIVE.md
 * §8) rather than calling apiClient directly — this file previously
 * bypassed the services/ layer, which was a real deviation from the
 * app's own architecture rule and has been fixed here.
 */
export function ProofOfDeliveryPage() {
  ...
  const handleSubmit = async () => {
    ...
    setError(null);
    try {
      await deliveryService.confirmDelivery(deliveryId, { method: "OTP", value: code });
