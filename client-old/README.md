...
- [`docs/api-contract.md`](docs/api-contract.md) — the exact request/
  response DTOs this frontend expects from every endpoint it calls,
  written for the backend team to implement against (per
  `FULL_SCALE_DELIVERY_DIRECTIVE.md` §12). This supersedes the endpoint
  guesses previously scattered as comments in `services/*.ts`.
...
**IMPLEMENTED:** `AuthProvider` (session bootstrap via `GET /auth/me` on
load), `useAuth`, `ProtectedRoute`, `RoleRoute`, `LoginPage`, `SignUpPage`
(role-based — see below), logout, role-aware redirect after login/signup,
Unauthorized (403) and Not Found (404) pages, and a client-side
**silent-refresh-and-retry**: any request that comes back `401` triggers
one `POST /auth/refresh` attempt (deduplicated if several requests 401 at
once — see `src/api/refreshCoordinator.ts`), retries the original request
once on success, and clears the session on failure. Endpoint paths for
login/refresh/logout/session-check are now aligned exactly to
`FULL_SCALE_DELIVERY_DIRECTIVE.md` §8: `POST /auth/login`, `POST
/auth/refresh`, `POST /auth/logout`, `GET /auth/me`.

**Sign-up (`SignUpPage`, `/signup`) is a frontend addition — `POST
/auth/register` is NOT in the directive's confirmed endpoint list.** It's
documented as a proposal in `docs/api-contract.md` for the backend team
to accept, modify, or reject. Design decisions worth knowing:
- **Role selection lives on sign-up only, never sign-in** — a role is
  established once and read from the account thereafter; asking a
  returning user to reassert it would be meaningless.
- **Admin roles are never offered at sign-up** — self-service admin
  creation is a privilege-escalation risk regardless of what the UI
  hides; admin accounts should be provisioned internally. The frontend
  omits the option, but the backend must independently reject an admin
  role sent directly to this endpoint — the frontend's omission is a UX
  choice, not the security boundary.
- **Retailer sign-up creates a new tenant** (`businessName` field).
  **Dispatcher/Rider sign-up joins an existing tenant via a backend-
  validated `organizationCode`** (an invite code), never a raw tenant ID
  — consistent with the app-wide rule that a tenant ID supplied by the
  browser is never treated as proof of access (§ Multi-Tenancy below).

**Still ASSUMED / PROVISIONAL** (see `docs/api-contract.md` for the full
list): exact payload field names/types for all of the above (the paths
are now confirmed, the shapes are still this app's best guess), refresh-
token rotation behavior, and whether sign-up needs an email/phone
verification step before first login.

**KNOWN GAP:** tokens are held in memory only (`state/authStore.ts`); a
hard page refresh currently logs the user out, since there's nothing
persisted to send to `GET /auth/me`. This is deliberate, not an
oversight — see `AuthProvider`'s docstring for why persisting a refresh
token client-side (e.g. localStorage) is a real security question that
shouldn't be decided silently; an httpOnly cookie the frontend never
touches directly is the safer default if the backend can support it.

Supported roles (confirmed set, `FULL_SCALE_DELIVERY_DIRECTIVE.md` §4):
`RETAILER`, `DISPATCHER`, `RIDER` (full screens, plus sign-up).
`SUPPORT_ADMIN`, `MANAGER_ADMIN`, `SYSTEM_ADMIN` exist in the `UserRole`
type and route tree (`/admin` → `AdminLayout`) as **architectural
placeholders only** — no admin screens are built, and none are offered
at sign-up.
...
**Endpoint paths are now confirmed** for the core flow —
`FULL_SCALE_DELIVERY_DIRECTIVE.md` §8 published them, and `services/*.ts`
has been updated to match exactly (see `docs/api-contract.md` for the
full per-endpoint request/response shapes, which is what's still
proposed, not the paths themselves). What's below is what's genuinely
still open — either because the directive doesn't cover it, or because a
path being confirmed doesn't mean its payload shape is.

- **Payload shapes:** every request/response body in `docs/api-contract.md`
  is this app's proposal, not a signed-off contract. The backend team
  needs to confirm or correct field names/types before this stops being
  "provisional."
- **`POST /auth/register`:** not in the directive at all — a frontend
  addition (see §8 above). Needs explicit backend sign-off on whether
  self-service sign-up is even wanted for MVP, plus whether a
  verification step (email/phone) should gate first login.
- **Refresh-token rotation:** assumed (directive §11 implies it via
  "DB-backed hashed refresh tokens") but not explicitly confirmed.
...
- **`GET /dispatch/...`:** the directive names this category but not an
  exact path. The Dispatch Center currently gets its queue from
  `GET /deliveries?status=...`, which works for MVP — see
  `docs/api-contract.md`'s open items for what a dedicated endpoint might
  need to add (SLA flags, priority scoring).
- **`GET /deliveries/:id/events`:** not in the directive's list at all —
  needed for the Timeline UI; flagged for confirmation or relocation
  (e.g. folded into the Delivery object itself).
- **Realtime transport:** Socket.IO, SSE, or none for MVP? See §9.
  Directive §7/§11 confirms Redis is "cache/coordination/queue," never
  system-of-record — doesn't by itself pick a transport.
- **Map provider:** Google Maps, Mapbox, or MapLibre? Pending ADR (cost vs.
  Kenya address-data quality trade-off, directive §11). See §10.
- **Payment rail:** is M-Pesa confirmed? Directive §11 flags "payment
  provider and webhook/idempotency strategy" as still open.
- **Proof of delivery:** which mechanism (OTP, QR, signature, photo)?
  `ProofOfDeliveryPage` implements a generic OTP-style placeholder.
- **Pagination envelope:** proposed shape is in `docs/api-contract.md`;
  directive §4 requires "one documented envelope" but doesn't supply it.
- **Rider location data:** live GPS or last-known/periodic pings? Affects
  `Rider.location` precision and dispatcher-map refetch frequency.
- **OpenAPI spec:** directive §8 says the backend "must publish an
  executable contract, preferably OpenAPI." Once that exists, it
  supersedes `docs/api-contract.md`, and generating frontend types from
  it becomes a near-term follow-up to retire the hand-maintained
  `PROVISIONAL` typing in `src/types/`.
...
containerization required for the frontend itself. When backend/database
work begins, the directive's §7 calls for Docker Compose for local
Postgres + Redis; that won't run on this machine. Whoever picks up
backend/infra work should plan for native Windows installs or a
cloud/managed dev instance of Postgres and Redis as the alternative,
rather than assuming Docker Compose is available to every contributor.

Every provisional type/service file has an inline comment marking it
`PROVISIONAL` or referencing `docs/api-contract.md` at the point of
assumption — search `src/` for those terms to find every place a
confirmed contract would change something.
...
- Auth: split-composition Sign In page (glass-pearl card, password
  show/hide, role-based post-login routing, login-specific error copy for
  401/403/network) plus a role-based Sign Up page (role-selector cards for
  Retailer/Dispatcher/Rider — never admin; Retailer creates a new tenant
  via business name, Dispatcher/Rider join an existing one via a
  backend-validated invite code, never a raw tenant ID), logout, protected
  routes, role-aware routing, session bootstrap via `GET /auth/me`,
...
- Realtime and Maps integration boundaries (interfaces + no-op/placeholder
  implementations)
- 36 passing tests across 9 files (Vitest + React Testing Library):
  ...
  StatusIndicator label/accessibility behavior, a full Sign In suite
  (renders, validation, loading, 401/403/network error mapping,
  role-based redirect for two different roles, password-visibility
  toggle, keyboard-reachability of the three key controls), and a full
  Sign Up suite (role-selector defaults and switching, admin roles never
  offered, required-field validation, password-mismatch rejection,
  RETAILER signup asserts `businessName` sent, RIDER signup asserts
  `organizationCode` sent and no `tenantId` ever sent, 409 "already
  exists" mapping, link back to Sign In)
